...
def get_financial_performance_data(filter_type: str = 'YTD',
    ...
    """
    Returns financial KPI metrics and chart data based on the selected date range filter.
    """
    ...
    with engine.connect() as conn:
        ...
        if filter_type == 'Daily':
            curr_avg = fetch_one(conn,
                "SELECT COALESCE(AVG(usd_value), 0) FROM transaction WHERE date_time::date = :d",
                {'d': start}
            )
        ...
        rows = conn.execute(text("""
            ...
            WHERE date_time::date BETWEEN :s AND :e
            GROUP BY a.name
            ORDER BY SUM((t.pricing_ic / 100.0) * t.usd_value + t.gateway_fee) / NULLIF(SUM(t.usd_value), 0) ASC
        """), {'s': start, 'e': end}).mappings().all()
